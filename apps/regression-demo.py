...
def _(BayesianRidge, np):
    ...
    return ConformalPrediction, MyBayesianRidge, optimize_polynomial_degree
...
def _(cho_factor, cho_solve, np):
    ...
    def optimize_gp_hyperparameters(X_train, y_train, kernel_type, initial_params, max_iter=50):
        ...
        if kernel_type == 'rbf':
            ...
            bounds = [(-2, 2), (-6, 0)]  # lengthscale: [0.14, 7.4], noise: [0.0025, 1.0]
    ...
    def fit_gp_numpy(X_train, y_train, X_test, kernel_type='rbf', use_polynomial_mean=False, poly_degree=None, **kernel_params):
        """
        ...
        **kernel_params : kernel hyperparameters
        ...
        y_std : array (n_test,) - GP uncertainty
        ...
        if use_polynomial_mean and poly_degree is not None:
            ...
            # Fit polynomial with Bayesian ridge regression (learns regularization from data)
        ...
        try:
            # Fit GP on residuals (or original data if no polynomial mean)
            ...
            if np.any(np.isnan(y_mean)) or np.any(np.isnan(y_std)):
                ...
                return np.zeros_like(X_test), np.ones_like(X_test), None

            # Return polynomial mean for plotting (None if not using it)
            return y_mean, y_std, (mean_test if use_polynomial_mean else None)
...
def _(
    ...
    for model_info in models_to_plot:
        ...
        if label == 'GP regression':
            # Fit GP with numpy
            y_pred, y_std, poly_mean = fit_gp_numpy(
                ...
                sigma=1.0,
                noise=sigma.value**2,
                use_polynomial_mean=get_gp_use_poly_mean(),
...
def _(
    ...
    if reg_enabled:
        ...
        aleatoric = mo.ui.checkbox(False, label="Include aleatoric uncertainty")
        bayes_opt_button_elem = bayes_optimize_button
    else:
        ...
        bayes_opt_button_elem = mo.Html(f"<div style='opacity: 0.4;'>{mo.ui.button(label='Optimize degree P', disabled=True)}</div>")
    ...
    if gp_regression.value:
        ...
        gp_use_poly_mean = mo.ui.checkbox(False, label="Use polynomial mean function", on_change=set_gp_use_poly_mean)
        gp_poly_mean_degree = mo.ui.slider(1, 15, 1, 3, label='Mean polynomial degree', on_change=set_gp_poly_mean_degree)
        ...
        gp_use_poly_mean = mo.Html(f"<div style='opacity: 0.4;'>{mo.ui.checkbox(False, label='Use polynomial mean function', disabled=True)}</div>")
    ...
    controls = mo.hstack([
        ...
        mo.vstack([reg_label, P_elem, aleatoric, bayes_opt_button_elem]),
        mo.vstack([cp_label, calib_frac, zeta]),
        mo.vstack([pops_label, percentile_clipping, leverage_percentile]),
        mo.vstack([gp_label, gp_kernel_dropdown, gp_lengthscale, gp_support_radius, gp_use_poly_mean, gp_poly_mean_degree, gp_opt_button_elem])
    ], gap=0.5)
...
def _(mo):
    ...
    return bayesian, conformal, gp_regression, pops


@app.cell(hide_code=True)
def _(mo):
    # Button definitions - isolated cell with no state dependencies to prevent circular loops
    bayes_optimize_button = mo.ui.run_button(label="Optimize degree P")
    gp_optimize_button = mo.ui.run_button(label="Optimize hyperparameters")
    return bayes_optimize_button, gp_optimize_button
...
def _(mo):
    ...
    get_gp_use_poly_mean, set_gp_use_poly_mean = mo.state(False)
...
def _(
    ...
    mo,
    np,
    optimize_polynomial_degree,
    seed,
    set_P,
    sigma,
):
    # Perform Bayesian polynomial degree optimization when button is clicked
    # Only run if button clicked AND P slider is available (regression enabled)
    mo.stop(not bayes_optimize_button.value or P_slider is None)
    ...
    _x_train = _x_train[(_x_train < 0) | (_x_train > 5.0)]
    _x_train = np.sort(_x_train)
    _y_train = g(_x_train, noise_variance=sigma.value**2)
    _X_train = _x_train[:, None]

    # Optimize polynomial degree
    try:
        optimal_degree, log_mls = optimize_polynomial_degree(
            _X_train, _y_train, max_degree=15
        )

        # Clamp to slider bounds [5, 15] and update
        optimal_P = np.clip(optimal_degree + 1, 5, 15)  # +1 because we use P as num params
        set_P(int(optimal_P))

        # Get the log ML for the optimal degree
        best_log_ml = max(log_mls, key=lambda x: x[1])[1]

        # Format output
        _output_lines = [
            "### ✓ Polynomial Degree Optimization Complete",
            f"**Log Marginal Likelihood:** {best_log_ml:.3f}",
            "",
            "**Optimized Parameters:**",
            f"- **Polynomial Degree:** {optimal_degree}",
            f"- **Number of Parameters (P):** {int(optimal_P)}"
        ]

        if optimal_degree + 1 != optimal_P:
            _output_lines.append(f"  - *(Clamped from {optimal_degree + 1} to slider bounds)*")
